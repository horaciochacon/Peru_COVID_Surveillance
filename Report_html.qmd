...
ggplot() +
  geom_line(data = ts_molecular, aes(x = week, y = PCR), col = "red") +
  geom_line(data = ts_positives, aes(x = week, y = Positives), col = "blue") +
  labs(
...
sus_pos_dpto %>% 
  select(
    Region = departamento,
    `Delay to lab result` = timediff_res_sympt_mean,
    `Delay to report` = timediff_sympt_cont_mean
  ) %>% 
  pander(
...
sus_pos %>% 
  ...
  summarise(
    timediff_res_sympt_mean = round(mean(timediff_res_sympt, na.rm = TRUE),2), 
    timediff_sympt_cont_mean = round(mean(timediff_sympt_cont, na.rm = TRUE),2),
  ) %>% 
...
sus_hos_dpto %>% 
  select(
    ...
    `Delay to attention` = timediff_mean
  ) %>% 
  pander(
    caption = "Main time delays (4a) by Region in average days",
...
```{r}
#| fig-cap: 
#| - "Timeliness indicators (4a)"
#| - "Timeliness indicators (4a) by diagnostic method"
